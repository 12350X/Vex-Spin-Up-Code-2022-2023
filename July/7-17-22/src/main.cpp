...
void drivercontrol() {
  ...
  printColored((char *)"Driver control active", green);
}
...
void auton() {
  ...
  Brain.Screen.clearScreen();
  printColored( (char *) "Autonomous initialized", green);
  autonomous a;
  a.prompt();
}
...
int main() {
  ...
  if (Brain.Battery.capacity() <
    ...
    printAtColored(0, 240, true, (char *)"Would you like to activate low power mode?", yellow); // Prompting user.
